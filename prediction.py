...
def transform(html):
    ...
    for div in interest_divs:
        ...
        span = div.find_all('span', class_='px-1 h-booklogosm font-bold bg-primary-yellow text-white leading-8 rounded-r-md w-14 md:w-18 flex justify-center items-center text-base')
...
def load(predicted_matches):
    for match, prediction in predicted_matches.items():
        print(BOLD + f'{match}' + END +
              f' -> Expected win for {prediction[min(prediction)]}')
